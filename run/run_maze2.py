...
# ✅ 載入 maze2.py
env_path = "C:/Users/seana/maze/env/maze2.py"
spec = importlib.util.spec_from_file_location("maze2", env_path)
maze2 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(maze2)

# ✅ 初始化環境
env = maze2.Maze2Env(render_mode="human")
state, _ = env.reset()
...
for s in range(1, env.rows * env.cols + 1):
    ...
    for i, a in enumerate(env.actions):
        ...
        nr, nc = (ns - 1) // env.cols, (ns - 1) % env.cols
        if ns != s and env.map[nr][nc] != 1:
            graph[s].append((ns, i))
...
save_path = f"C:/Users/seana/maze/outputs/path_maze2_{timestamp}.npy"
np.save(save_path, np.array(state_seq))
print(f"✅ agent 路徑已儲存至: {save_path}")

file_path = path("/mnt/data/run_maze2.py")
file_path.write_text(run_maze2_updated)
file_path.name
