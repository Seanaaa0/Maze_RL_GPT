...
class QDNGRU(nn.Module):
    def __init__(self, obs_shape, n_actions, seq_len=7):
        ...
        self.gru = nn.GRU(input_size=conv_out_dim,
    ...
    def forward(self, x_seq, hidden=None):
        ...
        # gru_out: [b, seq_len, 256]
        gru_out, hidden = self.gru(conv_out, hidden)
        return self.fc(gru_out[:, -1]), hidden  # 取最後一個時間步的輸出
